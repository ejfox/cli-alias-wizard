...
const exampleAliases = [
  ...
  },
  {
    ...
    description: "Switch git branches or restore working tree files",
  },
  {
    name: "ll",
    command: "ls -la",
  ...
  },
  {
    ...
    description: "Update and upgrade packages (Ubuntu/Debian)",
  },
];
...
function sourceConfig(configFile) {
  exec(`source ${configFile}`, { shell: process.env.SHELL }, (error) => {
    if (error) {
      console.log(
        `${cyber.error} Error sourcing configuration: ${error.message}`
      );
      console.log(
        `${cyber.bullet} Please run 'source ${configFile}' manually to use the new alias.`
      );
    } else {
      console.log(
        `${cyber.success} Configuration sourced successfully. New alias is ready to use.`
      );
    }
  });
}
...
async function useExampleAlias() {
  ...
  exampleAliases.forEach((alias, index) => {
    console.log(
      `${cyber.bullet} ${index + 1}. ${alias.name}: ${alias.description}`
    );
  });
  console.log(cyber.divider);
...
async function modifyExistingAlias() {
  ...
  const choice = await promptForInput(
    "Enter the number of the alias to modify:"
  );
  const index = parseInt(choice) - 1;
